#!/usr/bin/env python
"""
Script for viewing (or killing) long running postgres queries.

For more info, type: view-postgres-query -h
"""
import argparse
...
from cellsite.util.postgres import pgconnect
...
def list_tables(con):
    with con.cursor() as cur:
        ...
        print(
            tabulate(
                cur.fetchall(),
                headers=["schema", "table", "table size", "total size", "schema %"],
    ...
    with con.cursor() as cur:
        ...
        for pid, duration, query, datname, client_addr, blocked_by in cur.fetchall():
            if killall:
                ...
                blocked_str = (
                    ""
                    if not len(blocked_by)
                    else "; blocked by " + ", ".join([str(p) for p in blocked_by])
                )
                print(
                    f"[{pid}] running on {datname} from {client_addr} for {describe_interval(duration.total_seconds())}{blocked_str}: {query}"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=15,
        type=int,
        help=f"minimum age of listed queries (default: {DEFAULT_MINIMUM_AGE}).",
    )
    parser.add_argument(
        "--killall", help="kill all listed queries.", action="store_true"
    )
    parser.add_argument(
        "--kill", metavar="PID", help="kill a long running query.", type=int
    )
